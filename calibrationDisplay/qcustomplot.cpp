...
bool QCustomPlot::event(QEvent *event)
{
    ...
    if(!(mInteractions & QCP::iRangeZoom))
        return QWidget::event(event);
    if(event->type() == QEvent::TouchBegin)
    {
        foreach(QCPAxisRect *rect,axisRects())
        {
            rect->mDragStart = touchEvent->touchPoints()[0].pos().toPoint(); // need this even when not LeftButton is pressed, to determine in releaseEvent whether it was a full click (no position change between press and release)
            rect->mDragging = true;
            // initialize antialiasing backup in case we start dragging:
            if (rect->mParentPlot->noAntialiasingOnDrag())
            {
                rect->mAADragBackup = rect->mParentPlot->antialiasedElements();
                rect->mNotAADragBackup = rect->mParentPlot->notAntialiasedElements();
            }
            // Mouse range dragging interaction:
            if (rect->mParentPlot->interactions().testFlag(QCP::iRangeDrag))
            {
                if (rect->mRangeDragHorzAxis)
                    rect->mDragStartHorzRange = rect->mRangeDragHorzAxis.data()->range();
                if (rect->mRangeDragVertAxis)
                    rect->mDragStartVertRange = rect->mRangeDragVertAxis.data()->range();
            }
        }
        QMouseEvent *event = new QMouseEvent(QEvent::MouseButtonPress,touchEvent->touchPoints()[0].pos().toPoint(),Qt::LeftButton,Qt::LeftButton,Qt::NoModifier);
    ...
    else if(event->type() == QEvent::TouchUpdate)
    {
        if(touchEvent->touchPoints().count()==1)
        {
            foreach(QCPAxisRect *rect,axisRects())
            {
                if (rect->mDragging && rect->mParentPlot->interactions().testFlag(QCP::iRangeDrag))
                {
                    if (rect->mRangeDrag.testFlag(Qt::Horizontal))
                    {
                        if (QCPAxis *rangeDragHorzAxis = rect->mRangeDragHorzAxis.data())
                        {
                            if (rangeDragHorzAxis->mScaleType == QCPAxis::stLinear)
                            {
                                double diff = rangeDragHorzAxis->pixelToCoord(rect->mDragStart.x()) - rangeDragHorzAxis->pixelToCoord(touchEvent->touchPoints()[0].pos().toPoint().x());
                                rangeDragHorzAxis->setRange(rect->mDragStartHorzRange.lower+diff, rect->mDragStartHorzRange.upper+diff);
                    ...
                    if (rect->mRangeDrag.testFlag(Qt::Vertical))
                    {
                        if (QCPAxis *rangeDragVertAxis = rect->mRangeDragVertAxis.data())
                        {
                            if (rangeDragVertAxis->mScaleType == QCPAxis::stLinear)
                            {
                                double diff = rangeDragVertAxis->pixelToCoord(rect->mDragStart.y()) - rangeDragVertAxis->pixelToCoord(touchEvent->touchPoints()[0].pos().toPoint().y());
                                rangeDragVertAxis->setRange(rect->mDragStartVertRange.lower+diff, rect->mDragStartVertRange.upper+diff);
                    ...
                    if (rect->mRangeDrag != 0) // if either vertical or horizontal drag was enabled, do a replot
                    {
                        if (rect->mParentPlot->noAntialiasingOnDrag())
                            rect->mParentPlot->setNotAntialiasedElements(QCP::aeAll);
                        rect->mParentPlot->replot();
                    }
                }
            }
            replot();
        ...
        {
            double dx1 = touchEvent->touchPoints()[0].pos().x()-touchEvent->touchPoints()[1].pos().x();
            double dy1 = touchEvent->touchPoints()[0].pos().y()-touchEvent->touchPoints()[1].pos().y();
            double dx2 = touchEvent->touchPoints()[0].lastPos().x()-touchEvent->touchPoints()[1].lastPos().x();
            double dy2 = touchEvent->touchPoints()[0].lastPos().y()-touchEvent->touchPoints()[1].lastPos().y();
            double dis1 = sqrt(dx1*dx1 + dy1*dy1);
            double dis2 = sqrt(dx2*dx2 + dy2*dy2);
            double dis = dis1 - dis2;
            QPoint p;
            p.setX(touchEvent->touchPoints()[0].pos().x()+touchEvent->touchPoints()[1].pos().x()/2);
            p.setY(touchEvent->touchPoints()[0].pos().y()+touchEvent->touchPoints()[1].pos().y()/2);
            double factor;
            double wheelSteps = dis/20; // a single step delta is +/-120 usually
            foreach(QCPAxisRect *rect,axisRects())
            {
                if(rect->mDragging!=false)
                {
                    rect->mDragging = false;
                    if (rect->mParentPlot->noAntialiasingOnDrag())
                    {
                        rect->mParentPlot->setAntialiasedElements(rect->mAADragBackup);
                        rect->mParentPlot->setNotAntialiasedElements(rect->mNotAADragBackup);
                    }
                }
                if(rect->mRangeZoom!=0)
                {
                    if (rect->mRangeZoom.testFlag(Qt::Horizontal))
                    {
                        factor = pow(rect->mRangeZoomFactorHorz, wheelSteps);
                        if (rect->mRangeZoomHorzAxis.data())
                            rect->mRangeZoomHorzAxis.data()->scaleRange(factor, rect->mRangeZoomHorzAxis.data()->pixelToCoord(p.x()));
                    }
                    if (rect->mRangeZoom.testFlag(Qt::Vertical))
                    {
                        factor = pow(rect->mRangeZoomFactorVert, wheelSteps/5);
                        if (rect->mRangeZoomVertAxis.data())
                            rect->mRangeZoomVertAxis.data()->scaleRange(factor, rect->mRangeZoomVertAxis.data()->pixelToCoord(p.y()));
                    }
                }
            }
            replot();
...
//    painter->setBackground(mainBrush());
